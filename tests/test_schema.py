...
class TestCreateTaskSchema:
    ...
    def test_required_is_just_title(self) -> None:
    ...
    def test_brief_field_present_with_description(self) -> None:
    ...
    def test_no_unset_leak(self) -> None:
        import json
...
class TestUpdateTaskSchema:
    ...
    def test_required_taskid_projectid(self) -> None:
        assert set(self.schema["required"]) == {"taskId", "projectId"}

    def test_isallday_documents_explicit_rule(self) -> None:
        iso = self.schema["properties"]["isAllDay"]
        assert "explicitly" in iso["description"].lower()


class TestSchemaListing:
    def test_no_op_returns_listing(self) -> None:
