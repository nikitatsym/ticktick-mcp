...
async function exchangeCode(code, clientId2, clientSecret2) {
  const basicAuth = Buffer.from(`${clientId2}:${clientSecret2}`).toString("base64");
...
async function getAccessToken(clientId2, clientSecret2) {
  if (process.env.TICKTICK_ACCESS_TOKEN) {
    ...
    saveTokens(tokens2);
    return { accessToken: tokens2.access_token, tokens: tokens2 };
  }
  let tokens = loadTokens();
  if (tokens) {
    if (tokens.expires_at && Date.now() > tokens.expires_at - 6e4) {
      if (tokens.refresh_token && clientId2 && clientSecret2) {
        console.error("Access token expired, refreshing...");
        try {
          tokens = await refreshAccessToken(tokens.refresh_token, clientId2, clientSecret2);
          saveTokens(tokens);
  ...
  if (process.env.TICKTICK_AUTH_CODE && clientId2 && clientSecret2) {
    console.error("Exchanging auth code for tokens...");
    try {
      tokens = await exchangeCode(process.env.TICKTICK_AUTH_CODE, clientId2, clientSecret2);
      saveTokens(tokens);
...
var TICKTICK_TOKEN_URL, REDIRECT_URI, SCOPES, TOKEN_DIR, TOKEN_FILE;
var init_auth = __esm({
  "src/auth.js"() {
    TICKTICK_TOKEN_URL = "https://ticktick.com/oauth/token";
    REDIRECT_URI = "https://nikitatsym.github.io/ticktick-mcp/";
    SCOPES = "tasks:read tasks:write";
    TOKEN_DIR = path.join(process.env.HOME || process.env.USERPROFILE || "", ".ticktick-mcp");
    TOKEN_FILE = path.join(TOKEN_DIR, "tokens.json");
  }
});
...
// src/ticktick-client.js
var API_BASE = "https://api.ticktick.com/open/v1";
var TickTickClient = class {
  constructor(getToken, clientId2, clientSecret2) {
    this._getToken = getToken;
    this._clientId = clientId2;
    this._clientSecret = clientSecret2;
    this._accessToken = null;
    this._inboxId = null;
  }
  async _token() {
    if (!this._accessToken) {
      const { getAccessToken: getAccessToken2 } = await Promise.resolve().then(() => (init_auth(), auth_exports));
      const result = await getAccessToken2(this._clientId, this._clientSecret);
   ...
   */
  async _request(method, endpoint, body = void 0) {
    ...
    if (res.status === 401) {
      const { refreshAccessToken: refreshAccessToken2, loadTokens: loadTokens2, saveTokens: saveTokens2 } = await Promise.resolve().then(() => (init_auth(), auth_exports));
      const tokens = loadTokens2();
      if (tokens?.refresh_token && this._clientId && this._clientSecret) {
        try {
          const newTokens = await refreshAccessToken2(tokens.refresh_token, this._clientId, this._clientSecret);
          saveTokens2(newTokens);
...
// src/index.js
var log = (...args) => console.error("[ticktick-mcp]", ...args);
...
var clientId = process.env.TICKTICK_CLIENT_ID;
var clientSecret = process.env.TICKTICK_CLIENT_SECRET;
var client = new TickTickClient(null, clientId, clientSecret);
...
server.tool(
  ...
  "Create a new TickTick project (task list)",
  {
    ...
    color: external_exports.string().optional().describe('Color hex code, e.g. "#4772FA"'),
    viewMode: external_exports.enum(["list", "kanban", "timeline"]).optional().describe("View mode"),
    kind: external_exports.enum(["TASK", "NOTE"]).optional().describe("Project kind")
  },
...
server.tool(
  ...
  "Update an existing TickTick project",
  {
    ...
    color: external_exports.string().optional().describe("New color hex"),
    viewMode: external_exports.enum(["list", "kanban", "timeline"]).optional().describe("New view mode"),
    kind: external_exports.enum(["TASK", "NOTE"]).optional().describe("New kind")
  },
...
server.tool(
  ...
  { projectId: external_exports.string().describe("Project ID to delete") },
...
server.tool(
  ...
  {
    projectId: external_exports.string().describe("Project ID containing the task"),
...
server.tool(
  ...
  "Create a new task in TickTick. If projectId is omitted, the task goes to Inbox. Supports title, content, dates, priority (0=none, 1=low, 3=medium, 5=high), tags, subtasks (items), reminders, and recurrence (repeatFlag in iCal RRULE format). The response includes the assigned projectId (useful for getting the inbox ID).",
  {
    ...
    projectId: external_exports.string().optional().describe("Project ID (uses inbox if omitted)"),
    content: external_exports.string().optional().describe("Task content/notes (supports markdown)"),
    desc: external_exports.string().optional().describe("Task description"),
    startDate: external_exports.string().optional().describe('Start date in ISO 8601 format, e.g. "2026-02-18T09:00:00+0000"'),
    dueDate: external_exports.string().optional().describe("Due date in ISO 8601 format"),
    isAllDay: external_exports.boolean().optional().describe("Whether this is an all-day task"),
    priority: external_exports.number().optional().describe("Priority: 0=none, 1=low, 3=medium, 5=high"),
    tags: external_exports.array(external_exports.string()).optional().describe('Tags as array of strings, e.g. ["work", "urgent"]'),
    timeZone: external_exports.string().optional().describe('Time zone, e.g. "America/Los_Angeles"'),
    reminders: external_exports.array(external_exports.string()).optional().describe('Reminders in iCal trigger format, e.g. ["TRIGGER:PT0S", "TRIGGER:P0DT9H0M0S"]'),
    repeatFlag: external_exports.string().optional().describe('Recurrence rule in iCal RRULE format, e.g. "RRULE:FREQ=DAILY;INTERVAL=1"'),
    items: external_exports.array(
      external_exports.object({
        title: external_exports.string().describe("Subtask title"),
        status: external_exports.number().optional().describe("0=normal, 1=completed"),
        startDate: external_exports.string().optional().describe("Subtask start date"),
        isAllDay: external_exports.boolean().optional().describe("All-day subtask"),
        sortOrder: external_exports.number().optional().describe("Sort position"),
        timeZone: external_exports.string().optional().describe("Time zone")
      })
...
server.tool(
  ...
  "Update an existing task. Provide only the fields you want to change.",
  {
    ...
    content: external_exports.string().optional().describe("New content/notes"),
    desc: external_exports.string().optional().describe("New description"),
    startDate: external_exports.string().optional().describe("New start date (ISO 8601)"),
    dueDate: external_exports.string().optional().describe("New due date (ISO 8601)"),
    isAllDay: external_exports.boolean().optional().describe("All-day flag"),
    priority: external_exports.number().optional().describe("Priority: 0=none, 1=low, 3=medium, 5=high"),
    tags: external_exports.array(external_exports.string()).optional().describe("New tags"),
    timeZone: external_exports.string().optional().describe("Time zone"),
    reminders: external_exports.array(external_exports.string()).optional().describe("New reminders"),
    repeatFlag: external_exports.string().optional().describe("New recurrence rule"),
    items: external_exports.array(
      external_exports.object({
        title: external_exports.string().describe("Subtask title"),
        status: external_exports.number().optional().describe("0=normal, 1=completed"),
    ...
    ).optional().describe("Updated subtask/checklist items")
  },
...
server.tool(
  ...
  {
    projectId: external_exports.string().describe("Project ID"),
    taskId: external_exports.string().describe("Task ID to delete")
  },
  ...
  "Create multiple tasks at once. Each task object supports the same fields as create_task.",
  {
    tasks: external_exports.array(
      external_exports.object({
        ...
        content: external_exports.string().optional().describe("Content/notes"),
        desc: external_exports.string().optional().describe("Description"),
        startDate: external_exports.string().optional().describe("Start date (ISO 8601)"),
        dueDate: external_exports.string().optional().describe("Due date (ISO 8601)"),
        isAllDay: external_exports.boolean().optional().describe("All-day task"),
        priority: external_exports.number().optional().describe("Priority: 0/1/3/5"),
        tags: external_exports.array(external_exports.string()).optional().describe("Tags"),
        timeZone: external_exports.string().optional().describe("Time zone"),
        reminders: external_exports.array(external_exports.string()).optional().describe("Reminders"),
        repeatFlag: external_exports.string().optional().describe("Recurrence rule"),
        items: external_exports.array(
          external_exports.object({
            title: external_exports.string(),
            status: external_exports.number().optional(),
            startDate: external_exports.string().optional(),
            isAllDay: external_exports.boolean().optional(),
            sortOrder: external_exports.number().optional(),
            timeZone: external_exports.string().optional()
          })
        ).optional().describe("Subtasks")
      })
